#!/usr/bin/env python
"""Finalize causal lightweight alignment tokenizer suite summaries."""
...
def best_empirical_value(empirical: dict[str, Any], key: str) -> float | None:
    ...
    for item in empirical.get("per_lag", []):
        value = item.get("leave_one_subject_out", {}).get("accuracy_gain") if key == "loso_gain" else item.get(key)
...
def downstream_summary(suite_dir: Path, run_name: str) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        if task in {"nback_load_0_vs_2_vs_3", "motor_lmi_vs_rmi", "nback_vs_wg"}:
    ...
    fine_values = [
        ...
        if key.endswith("_balanced_accuracy") and (
            "nback_load_0_vs_2_vs_3" in key or "motor_lmi_vs_rmi" in key
        )
    ]
...
def row_for_run(suite_dir: Path, run_dir: Path, stage: str) -> dict[str, Any]:
    ...
    row = {
        ...
        "best_val_interaction_aux_loss": best_metrics.get("val_interaction_aux_loss"),
        "best_val_shared_state_bottleneck_loss": best_metrics.get("val_shared_state_bottleneck_loss"),
...
def collect_rows(suite_dir: Path) -> list[dict[str, Any]]:
    ...
    for stage, leaf in (("formal", "tokenizer_interventions"), ("smoke", "smoke")):
        for run_dir in sorted((suite_dir / leaf).glob("*causal_alignment_seed*")):
...
def add_baseline_deltas(rows: list[dict[str, Any]]) -> None:
    baseline = {
        (row["stage"], row["seed"]): row
        for row in rows
        if row.get("condition") == "A0"
    }
    keys = [
    ...
    for row in rows:
        ...
        for key in keys:
            row[f"{key}_delta_vs_a0"] = None if base is None else relative_change(row.get(key), base.get(key))
        if base is None:
            row["downstream_best_fine_balanced_accuracy_abs_delta_vs_a0"] = None
        else:
            value = row.get("downstream_best_fine_balanced_accuracy")
            base_value = base.get("downstream_best_fine_balanced_accuracy")
            row["downstream_best_fine_balanced_accuracy_abs_delta_vs_a0"] = (
...
def decision_from_summary(summary: dict[str, Any]) -> dict[str, Any]:
    ...
    for condition, payload in conditions.items():
        if condition == "A0":
            continue
        info_pass = (
            (payload.get("info_hard_cmi_delta_vs_a0_mean") or 0.0) >= 0.25
            or (payload.get("info_hard_loso_ridge_delta_vs_a0_mean") or 0.0) >= 0.25
        )
        fine_pass = (payload.get("downstream_best_fine_balanced_accuracy_abs_delta_vs_a0_mean") or 0.0) >= 0.03
        coupling_pass = (payload.get("gate3_best_mi_above_shuffle_delta_vs_a0_mean") or 0.0) >= 0.20
        gradient_pass = (
            payload.get("grad_alignment_rec_cosine_mean_mean") is None
            or float(payload.get("grad_alignment_rec_cosine_mean_mean")) >= -0.2
        )
        token_health_pass = (
            (payload.get("best_val_primary_loss_delta_vs_a0_mean") is None or payload.get("best_val_primary_loss_delta_vs_a0_mean") <= 0.05)
            and (payload.get("best_val_eeg_source_perplexity_delta_vs_a0_mean") is None or payload.get("best_val_eeg_source_perplexity_delta_vs_a0_mean") >= -0.20)
            and (payload.get("best_val_fnirs_source_perplexity_delta_vs_a0_mean") is None or payload.get("best_val_fnirs_source_perplexity_delta_vs_a0_mean") >= -0.20)
        )
        ...
        if info_pass and fine_pass and gradient_pass and token_health_pass:
            promoted.append(condition)
    if any(condition in promoted for condition in ("A3", "A4")):
        next_step = "promote lightweight alignment with coupling prior; extend candidate to 80 epochs"
    elif "A2" in promoted:
        next_step = "promote direct-gradient latent alignment; keep coupling as audit until it passes"
    elif promoted:
        next_step = "inspect promoted diagnostic condition manually before architecture changes"
    else:
        next_step = "do not promote lightweight alignment yet; inspect smoke/formal failures and consider stronger information exchange"
    return {
        "status": "complete",
...
def main() -> None:
    ...
    summary = {
        "schema_version": "tokenizer_lightweight_alignment_summary_v1",
        "suite_dir": str(suite_dir),
        "categories": {
            "information_retention": [
                "info_hard_cmi",
                "info_hard_loso_ridge",
                "info_continuous_to_soft_retention",
                "info_soft_to_hard_drop",
            ],
            "fine_task_probe": [
                "downstream_best_fine_balanced_accuracy",
                "downstream_nback_load_0_vs_2_vs_3_bot_balanced_accuracy",
                "downstream_motor_lmi_vs_rmi_bot_balanced_accuracy",
            ],
            "gate3_coupling": [
                "gate3_best_mi_above_shuffle",
                "gate3_best_loso_gain",
            ...
            "gradient_conflict": [
                "grad_alignment_rec_cosine_mean",
                "grad_alignment_share_mean",
            ],
            "token_health": [
                "best_val_primary_loss",
                "best_val_eeg_source_perplexity",
    ...
    for condition, payload in summary["conditions"].items():
        lines.append(
            ...
            f"primary_delta={payload.get('best_val_primary_loss_delta_vs_a0_mean')}, "
            f"info_cmi={payload.get('info_hard_cmi_mean')}, "
            f"info_delta={payload.get('info_hard_cmi_delta_vs_a0_mean')}, "
            f"fine_acc={payload.get('downstream_best_fine_balanced_accuracy_mean')}, "
            f"fine_abs_delta={payload.get('downstream_best_fine_balanced_accuracy_abs_delta_vs_a0_mean')}, "
            f"MI={payload.get('gate3_best_mi_above_shuffle_mean')}, "
            f"MI_delta={payload.get('gate3_best_mi_above_shuffle_delta_vs_a0_mean')}, "
