#!/usr/bin/env python
"""Launch causal pre-VQ cross-modal exchange tokenizer experiments."""
...
SUITE_PARENT = PROJECT_ROOT / "experiments/runs/tokenizer_cross_modal_exchange"
NEXT_STAGE_SUITE = (
    PROJECT_ROOT
...
LIGHTWEIGHT_SUITE = (
    PROJECT_ROOT
    / "experiments/runs/tokenizer_lightweight_alignment/20260624_173228_causal_pre_vq_alignment_v1"
)
MAX_LAG_TOKENS = 5
...
CONDITIONS = (
    ExchangeCondition("X0", "no-exchange baseline continuation"),
    ExchangeCondition(
        "X1",
        "detached causal EEG-to-fNIRS pre-VQ source adapter",
        exchange_enabled=True,
        detach_context=True,
    ),
    ExchangeCondition(
        "X2",
        "direct-gradient causal EEG-to-fNIRS pre-VQ source adapter",
        exchange_enabled=True,
        detach_context=False,
    ),
    ExchangeCondition(
        "X3",
        "direct-gradient causal adapter plus weak global residual coupling prior",
        exchange_enabled=True,
        detach_context=False,
    ...
    ExchangeCondition(
        "X4",
        "direct-gradient causal adapter plus context residual coupling prior",
        exchange_enabled=True,
        detach_context=False,
        coupling_weight=0.01,
...
def base_for_seed_index(seed_index: int) -> dict[str, Any]:
    return base_specs()[seed_index % len(base_specs())]


def run_name_for(condition: ExchangeCondition, seed: int) -> str:
    return f"k128_dim128_{condition.key.lower()}_causal_exchange_seed{seed}"


def wait_for_lightweight_audit_command() -> str:
    audit_script = LIGHTWEIGHT_SUITE / "queue_logs/audit_queue.sh"
    done = LIGHTWEIGHT_SUITE / "queue_logs/audit.done"
    return (
        f"if [ -e {q(audit_script)} ] && [ ! -e {q(done)} ]; then "
        f"while pgrep -f {q(str(audit_script))} >/dev/null; do sleep 300; done; "
        "fi"
    )
    ...
    *,
    ...
    cfg: dict[str, Any] = {
        ...
        "experiment": {
            "name": run_name,
            "run_group": (
                f"tokenizer_cross_modal_exchange/{suite_name}/"
        ...
        "model": {
            "cross_modal_exchange": {
                "enabled": condition.exchange_enabled,
                ...
                "detach_context": condition.detach_context,
                "residual_init": 0.1,
                "dropout": 0.05,
            },
        },
        ...
        "loss": {
            ...
            "shared_state_bottleneck": {"weight": 0.0, "dim": 32, "stop_gradient": True},
        },
        "training": {
            "batch_size": 256,
            ...
            "validation": {"interval_epochs": 1, "start_epoch": 1, "max_batches": 2 if smoke else None},
...
def training_command(config: Path, run_name: str, *, smoke: bool) -> str:
    ...
    return (
        "bash experiments/scripts/archive/pre_physiology_semantic_20260701/launch_legacy_training_nohup.sh --task source-observation-tokenizer "
...
def audit_commands(suite_dir: Path, device: str) -> list[str]:
    ...
    for run_dir in sorted((suite_dir / "tokenizer_interventions").glob("k128_dim128_x*_causal_exchange_seed*")):
        label = run_dir.name
        checkpoint = run_dir / "checkpoints/best_model.pt"
        ...
        commands.extend([
            maybe(
                ".venv/bin/python experiments/scripts/archive/pre_physiology_semantic_20260701/export_coupling_audit_data.py "
            ...
            maybe(
                ".venv/bin/python experiments/scripts/archive/pre_physiology_semantic_20260701/analyze_coupling_identifiability.py "
            ...
            maybe(
                ".venv/bin/python experiments/scripts/archive/pre_physiology_semantic_20260701/analyze_codebook_geometry.py "
                f"--checkpoint {q(checkpoint)} --output-dir {q(geometry_dir)} --run-name {q(label)}",
                geometry_dir / "summary.json",
            ),
            maybe(
                ".venv/bin/python experiments/scripts/archive/pre_physiology_semantic_20260701/analyze_information_drop.py "
            ...
            maybe(
                ".venv/bin/python experiments/scripts/archive/pre_physiology_semantic_20260701/analyze_local_coupling.py "
                f"--export-dir {q(export_dir)} --output-dir {q(local_dir)} --run-name {q(label)} "
                f"--max-lag-tokens {MAX_LAG_TOKENS}",
                local_dir / "summary.json",
            ),
            maybe(
                "bash experiments/scripts/archive/pre_physiology_semantic_20260701/launch_legacy_training_nohup.sh --task source-observation-token-export --foreground "
            ...
            maybe(
                ".venv/bin/python experiments/scripts/archive/pre_physiology_semantic_20260701/run_source_observation_token_downstream_probe.py "
                f"--token-run-dir {q(token_export)} --output-dir {q(downstream_dir)}",
                downstream_dir / "summary.json",
            ),
        ])
    commands.append(f".venv/bin/python experiments/scripts/archive/pre_physiology_semantic_20260701/finalize_tokenizer_cross_modal_exchange_suite.py --suite-dir {q(suite_dir)}")
    return commands
...
def initialize_suite(suite_dir: Path, *, formal_epochs: int, seeds: tuple[int, int]) -> dict[str, Any]:
    ...
    formal_by_gpu = {0: [wait_for_lightweight_audit_command()], 1: [wait_for_lightweight_audit_command()]}
    smoke_by_gpu = {0: [], 1: []}
    ...
    manifest = {
        "schema_version": "tokenizer_cross_modal_exchange_suite_v1",
        ...
        "warm_starts": [{key: str(value) for key, value in spec.items()} for spec in base_specs()],
        "conditions": [asdict(condition) for condition in CONDITIONS],
        "formal_epochs": int(formal_epochs),
        "seeds": list(seeds),
        "max_lag_tokens": MAX_LAG_TOKENS,
        "lightweight_audit_wait_target": str(LIGHTWEIGHT_SUITE / "queue_logs/audit.done"),
        "queues": {
...
def main() -> None:
    ...
    parser.add_argument("--seeds", nargs=2, type=int, default=(20260651, 20260652))
    args = parser.parse_args()

    suite_dir = Path(args.suite_dir).resolve() if args.suite_dir else (
        SUITE_PARENT / f"{datetime.now().strftime('%Y%m%d_%H%M%S')}_causal_cross_adapter_v1"
    )
