...
def audit_commands(suite_dir: Path) -> list[str]:
    ...
    for spec in run_specs():
        ...
        commands.append(
            f".venv/bin/python experiments/scripts/archive/pre_physiology_semantic_20260701/analyze_codebook_geometry.py --checkpoint {q(checkpoint)} "
            f"--output-dir {q(suite_dir / 'codebook_geometry' / name)} --run-name {q(name)}"
        )
        if spec["key"] in {"K64", "K128", "K256"}:
            commands.append(
                f".venv/bin/python experiments/scripts/archive/pre_physiology_semantic_20260701/analyze_information_drop.py --checkpoint {q(checkpoint)} "
            ...
            commands.append(
                f".venv/bin/python experiments/scripts/archive/pre_physiology_semantic_20260701/analyze_local_coupling.py --export-dir {q(export_dir)} "
                f"--output-dir {q(suite_dir / 'local_coupling_audit' / name)} --run-name {q(name)} "
                f"--max-lag-tokens {MAX_LAG_TOKENS}"
            )
    commands.append(f".venv/bin/python experiments/scripts/archive/pre_physiology_semantic_20260701/finalize_tokenizer_coupling_capacity_suite.py --suite-dir {q(PREVIOUS_SUITE)}")
    commands.append(f".venv/bin/python experiments/scripts/archive/pre_physiology_semantic_20260701/finalize_tokenizer_coupling_discovery_suite.py --suite-dir {q(suite_dir)}")
    return commands
