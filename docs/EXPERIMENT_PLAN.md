...
| --- | --- | --- | --- | --- |
| Main method | [`SSM reliability`](analysis/SSM_RECONSTRUCTION_RELIABILITY_PLAN.md) → [`continuous latent`](analysis/CONTINUOUS_SHARED_PRIVATE_LATENT_PLAN.md) | The previous VQ/shared-driver generation remains unsupported; SSM reliability S0 is running and the no-VQ continuous latent experiment is planned | Complete S0, run the bounded MA/WG S1 audit, then start continuous shared/private validation | [`PROJECT_STATUS.md#主方法`](PROJECT_STATUS.md#主方法), [`SSM plan`](analysis/SSM_RECONSTRUCTION_RELIABILITY_PLAN.md), [`continuous plan`](analysis/CONTINUOUS_SHARED_PRIVATE_LATENT_PLAN.md) |
