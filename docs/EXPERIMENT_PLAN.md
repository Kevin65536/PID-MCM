...
| --- | --- | --- | --- | --- |
| Main method | [`SSM reliability`](analysis/SSM_RECONSTRUCTION_RELIABILITY_PLAN.md) → [`continuous latent`](analysis/CONTINUOUS_SHARED_PRIVATE_LATENT_PLAN.md) | SSM reliability completed 279/279 subject/task cells: joint is a qualified posterior-fit diagnostic, while EEG-only cross-modal recovery is unreliable; the no-VQ continuous latent experiment is planned | Implement continuous shared/private latent with matched-swap, derangement, private-only, and shared-only controls | [`PROJECT_STATUS.md#主方法`](PROJECT_STATUS.md#主方法), [`SSM results`](analysis/20260819_SSM_RECONSTRUCTION_RELIABILITY_RESULTS.md), [`continuous plan`](analysis/CONTINUOUS_SHARED_PRIVATE_LATENT_PLAN.md) |
