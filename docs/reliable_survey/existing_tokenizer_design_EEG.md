...
| Model | Preprocessing | Patch Length | Token Size (Original Signal) | Tokenizer | Tokenizer Params |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **LaBraM** | **0.1-75 Hz** BP, 50 Hz Notch, **200 Hz** Resample, µV | **200 points** (1 s) | **200 points** (1 channel, 1 s) | **VQ-NSP** (Frequency & Phase) | Codebook: **8192**<br>Dim: **64** |
| **NeuroLM** | **0.1-75 Hz** BP, 50/60 Hz Notch, **200 Hz** Resample, µV | **200 points** (1 s) | **200 points** (1 channel, 1 s) | **VQ-Align** (Freq & Raw) | Codebook: **8192**<br>Dim: **128** |
| **NeuroRVQ** | **0.5-45 Hz** BP, 50/60/100 Hz Notch, **200 Hz** Resample | **200 points** (1 s) | **200 points** (1 channel, 1 s) | **VQ** (Multi-scale) | Codebook: **8192**<br>Dim: **128** |
| **Neuroformer**| *Spike/Frame based (Not continuous signal patching)* | N/A (Uses spike bins/ID) | N/A | Transformer (Spike/Frame) | Context: ~15-700 tokens |
...
*   **Preprocessing**:
    ...
    *   Unit: µV
    *   Reference: `README.md`
*   **Signal Patch & Token Size**:
    *   **Patch Size**: 200 time points.
    *   **Token Correspondence**: 1 Token corresponds to **1 patch (200 points)** of a single channel.
    *   At 200 Hz, this equals **1.0 second** of data per token.
    *   Reference: `modeling_pretrain.py`, `modeling_vqnsp.py` (`patch_size=200`, `rearrange(..., T=200)`).
*   **Tokenizer**:
    *   Method: **VQ-NSP** (Vector-Quantized Neural Spectrum Prediction). Reconstructs Amplitude and Phase of FFT.
    *   Parameters: `n_code=8192`, `code_dim=64`.
    *   Reference: `README.md`, `modeling_vqnsp.py`.

### 2. NeuroLM
*   **Preprocessing**:
    *   Same as LaBraM (referenced in README).
    *   Bandpass: 0.1 – 75 Hz
    *   Notch: 50/60 Hz
    *   Resample: 200 Hz
    *   Reference: `README.md`.
*   **Signal Patch & Token Size**:
    *   **Patch Size**: 200 time points.
    *   **Token Correspondence**: 1 Token corresponds to **1 patch (200 points)** of a single channel.
    *   Reference: `dataset.py` (`rearrange(data, 'N (A T) -> (A N) T', T=200)`), `model/model_vq.py` (`decoder_out_dim=200`).
*   **Tokenizer**:
    *   Method: **VQ-Align**. Reconstructs Frequency Spectrum and Raw Time-domain signal.
    *   Parameters: `n_embed=8192`, `embed_dim=128`.
    *   Reference: `model/model_vq.py` defaults.

### 3. NeuroRVQ
*   **Preprocessing**:
    *   Notch: 50, 60, 100 Hz.
    *   Bandpass: 0.5 – 45 Hz (different from LaBraM/NeuroLM).
    *   Resample: 200 Hz.
    *   Clip: 500 µV.
    *   Reference: `preprocessing/preprocessing_eeg_example.py`.
*   **Signal Patch & Token Size**:
    *   **Patch Size**: 200 time points (defined in config).
    *   **Token Correspondence**: 1 Token corresponds to **1 patch (200 points)**.
    *   Reference: `flags/NeuroRVQ_EEG_v1.yml`.
*   **Tokenizer**:
    *   Method: Multi-scale VQ.
    *   Parameters: `n_code=8192`, `code_dim=128`.
    *   Reference: `flags/NeuroRVQ_EEG_v1.yml`.

### 4. Neuroformer
*   **Nature**: Multimodal (Spikes, Video Frames, Behavior) Generative Model.
*   **Signal Input**: Takes `spikes` (N_neurons x Time) and `frames`. Does not appear to use the same "continuous signal patching" strategy as the EEG models above.
*   **Config**: `configs/Visnav/lateral/mconf_pretrain.yaml` shows block sizes for `behavior`, `frame`, `id` (spike ID), but no signal patch size.
*   **Relevance**: This model operates on a different modality paradigm (Population vectors/Spike trains) compared to the EEG Foundation Models.
