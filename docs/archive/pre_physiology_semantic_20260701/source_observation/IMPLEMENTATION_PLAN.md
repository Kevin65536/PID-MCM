...
> Superseded for future implementation by: [active implementation plan](../../../physiology_semantic_tokenizer/04_IMPLEMENTATION_VALIDATION_PLAN.md)
> Detailed historical rationale: [PHYSIOLOGICAL_COUPLING_PLAN.md](PHYSIOLOGICAL_COUPLING_PLAN.md)
> Historical scorecard: [SEMANTIC_TOKEN_SCORECARD.md](SEMANTIC_TOKEN_SCORECARD.md) — 4 evaluation gates (Health / Semantics / Structure / Utility)
> Historical experiment log: [EXPERIMENT_LOG.md](EXPERIMENT_LOG.md)
> Runnable compatibility architecture: [docs/ARCHITECTURE.md](../../../ARCHITECTURE.md)

---

## 1. Role of This Document

本文件保留 source/observation 主线的历史实现顺序；它不再是未来 tokenizer 改造的权威计划。新架构以 [physiology-semantic implementation plan](../../../physiology_semantic_tokenizer/04_IMPLEMENTATION_VALIDATION_PLAN.md) 为准。
...
3. branch target 的当前合同是：为 EEG 与 fNIRS 分别构造**联合约束的干净生理成分**，且两者必须处于各自测量空间中；
4. observation target 的当前合同是：逐模态满足 `raw_signal = source_target + observation_target`，因此 residual 必须可线性定义为 `raw - source_target`；
5. 双 decoder 架构保持不变：source/observation 各有独立 decoder，full = source_recon + obs_recon（加法组合）；
6. Croce 风格联合状态空间模型仍是重要候选家族，但旧的单向或代理式 target-construction 路径不再被视为当前 branch semantics；
7. Coupling 结构先验仍只服务于 source correspondence，不再被当作 source target 语义本身的替代品。

### 2.2.1 Current Branch Target Contract

当前主线只接受满足以下条件的 branch target 定义：

1. **联合约束**：source target 不能由单一模态单向构造，必须由 EEG 与 fNIRS 同时约束；
2. **双边对称**：必须同时给出 clean EEG target 与 clean fNIRS target，且两者拥有同等的物理地位；
3. **测量空间输出**：source branch 监督对象应是干净观测成分，而不是只在 latent 中自洽的隐变量；
4. **线性残差可用**：source target 必须与原始测量保持足够时间同步和单位一致性，使 `observation_target = raw - source_target` 成立；
5. **nuisance 可分离**：被试差异、导联接触、仪器漂移和其他观测污染应尽量被压到 observation branch，而不是混入 source 定义。

因此，下列表述不再属于当前 branch target 合同：

1. 任何单向跨模态 proxy 作为 source target 定义；
2. 把任何单模态幅值代理本身当作 source semantics；
3. 通过对原始 fNIRS 做 per-channel mean/std 对齐来“制造” source target；
4. 用 coupling 可预测性替代 clean source target 的显式定义。

### 2.3 Control policy

对照面由以下两类提供：

1. **外部研究方法对照**：来自 [comparative_methods](../../../../comparative_methods) 和历史 `reference_repository` 的方法实现或复现结果；
...
顶层 [docs](../../..) 只保留当前主线需要反复阅读的活跃文档；历史材料统一进入 [docs/archive](../..)。历史预实验记录和 shared/private 阶段设计文档通过 git 历史查阅，不再维护独立的归档文档文件。

架构修改记录体系：每次架构变更必须先在 [docs/architecture_changelog/](../../../architecture_changelog/) 中建立独立记录（模板见 [template.md](../../../architecture_changelog/template.md)），然后更新 [docs/ARCHITECTURE.md](../../../ARCHITECTURE.md) 到新的当前状态。
...
shared/private → source/observation 语义切换，双独立 source codebook 替换单一 shared quantizer，删除所有旧 loss 项。详见 [2026-05-06 changelog](../../../architecture_changelog/2026-05-06_source_observation_migration.md)。

**成果**：Gate 1 (Health) 稳定通过；baseline locked at `gate1_best_current.yaml`。

### 5.2 Phase 2: Historical Proxy-Target Introduction ⚠️

这一阶段曾用 double-gamma HRF kernel 和若干 proxy target 替代 smooth proxy。详见 [2026-05-11 changelog](../../../architecture_changelog/2026-05-11_phase2a_branch_target_redesign_dual_decoder.md)。

**当前解读**：这是一个历史近似阶段，不再代表现阶段的 branch target 合同。其价值主要在于暴露了“单向 proxy target + 非对称 source semantics”会如何破坏 Gate 2-4。
...
### 5.4 Phase 2B: Croce Candidate Model Audit ⚠️

这一阶段将 Croce et al. 2017 及其若干 proxy 化实现引入仓库，建立了联合状态空间模型与 SMC 验证工具链。详见 [2026-05-13 changelog](../../../architecture_changelog/2026-05-13_phase2b_croce2017_physical_model_targets.md)。

**当前解读**：这一步提供了重要的候选模型和验证基础设施，但并没有最终确立现阶段的 branch target 合同。相关实现应被视为待比较的近似求解路径，而不是当前语义定义本身。
...
### 5.6 Phase 2C: Croce Local Cache Generation + HighWL-Only Tokenizer Input ✅

完成了两项关键基础设施工作：

1. **Croce 局部缓存生成**：对两个数据集（EEG-fNIRS Single Trial + Simultaneous EEG&NIRS），以 fNIRS spatial anchor 为单位运行 Croce 2017 particle filter，生成显式 source/observation target 缓存。每个样本为局部邻域：EEG `[B,6,4000]`，fNIRS `[B,1,200]`，附带显式 clean EEG / clean fNIRS source target 和线性 residual observation target。

2. **HighWL-only tokenizer 输入**：当前训练阶段 fNIRS 仅使用 `highWL`（`source_fnirs_optical_channel_0` / `obs_fnirs_optical_channel_0`），作为 HbO-sensitive optical measurement-space 代理。`lowWL` 保留于缓存元数据中但暂不参与模型输入。

详见 [2026-06-04 changelog](../../../architecture_changelog/2026-06-04_highwl_croce_local_tokenizer_input.md)。

关键交付物：
- [src/data/croce_local_cache_dataset.py](../../../../src/data/croce_local_cache_dataset.py) — Croce local cache 数据适配器
- `experiments/configs/source_observation/croce_local/` — 活跃训练配置（highwl_base / highwl_lr2e4 / highwl_fnirsobs64）
- Gate0 highWL-only 缓存/输入合同（见 [SEMANTIC_TOKEN_SCORECARD.md](SEMANTIC_TOKEN_SCORECARD.md)）
...
experiments/configs/source_observation/
   ...
   phase2a/          # Phase 2A dual decoder + spatial source targets（历史参考）
   croce_local/      # Phase 2C Croce local highWL-only tokenizer（活跃）
```
...
2. `source_observation_analysis.py` — source/observation 对齐分析与 Gate 0-4 scorecard 汇总（Gate0 验证 highWL-only 缓存/输入合同）；
3. 手动 rerun 入口：
...
### 6.3 Source/observation targets

当前主线使用 Croce local cache 提供的显式 source/observation target（见 §5.6）。每个样本携带：
- EEG source target `[B,6,4000]` + fNIRS highWL source target `[B,1,200]`
- EEG obs target `[B,6,4000]` + fNIRS highWL obs target `[B,1,200]`
- `obs_target = raw - source_target`（逐模态线性残差）

`src/data/channel_adjacency.py` 提供 10-10 EEG 邻居表、fNIRS 通道名解析、mnt.mat 3D 坐标校验、邻接矩阵构建和可视化，用于缓存生成阶段的局部邻域定义和空间分析，不再直接计算 tokenizer 训练中的 branch target。
...
所有正式实验统一按 4-gate 体系评价。Gate 定义详见 [SEMANTIC_TOKEN_SCORECARD.md](SEMANTIC_TOKEN_SCORECARD.md)。

| Gate | 回答的问题 | 当前要求 |
|------|-----------|----------|
| **Gate 0: Contract** | 当前 run 是否使用了约定的 highWL-only Croce local cache？ | fNIRS 为 highWL；lowWL 标记为 ignored；pair_mode="wavelength"；1 spatial anchor × 1 optical component |
| **Gate 1: Health** | codebook 是否健康？reconstruction 是否收敛？ | 4 个 quantizer 均满足健康阈值；full recon 收敛 |
| **Gate 2: Semantics** | source/observation 是否在做各自该做的事？ | cached source target 收敛；obs gap > 0；cross-modal predictability > chance |
...
(Phase2C) (Phase 1) (Phase 2A) (Phase 2B) (Phase 4+)
```

Gate 1 和 Gate 2 已在历史 proxy-target 条件下通过。当前 Phase 2C 使用 Croce local cache target 重新验证 Gate 1-4。Gate 0 是 highWL-only 训练阶段的硬性前置条件。
...
11. ~~Phase 2C: Croce local cache 生成（两个数据集）~~ ✅
12. ~~Phase 2C: HighWL-only tokenizer 输入 + Gate0 合同~~ ✅
13. **启动 highWL-only Croce local tokenizer 训练 — 当前焦点**
14. 在 Croce local cache 上验证 Gate 1-4
15. 统一导出主线与外部方法对照结果到同一 summary schema
16. tokenizer 证据充分后，再考虑 foundation model 层面的目标替换
...
8. 本文件中的状态更新；
9. [架构修改日志](../../../architecture_changelog/INDEX.md) 条目（含 before/after 架构图、组件变更表、设计决策）；
10. 更新后的 [ARCHITECTURE.md](../../../ARCHITECTURE.md)（反映变更后的当前架构状态）。
...
1. Coupling-aware quantization（原 Phase 2C core）：在量化步骤中消费 coupling 结构先验
2. HRF-shaped lag weighting
3. Foundation model 预训练目标的大改
4. Equal token count per window 的结构审计
5. HRF 频带分解（delta/theta/alpha/beta/gamma）
6. 重新引入 identity-style alignment losses

### Abandoned（已废弃，不再计划实现）

1. **把单向方向性当作 branch target 语义本身** — 无论是 `EEG → shared state → HRF → fNIRS` 还是其他单向 proxy，只要不能同时给出同等地位的 clean EEG 与 clean fNIRS target，就不再属于当前主线定义。
...
当前项目的 tokenizer 主线已经完成从”被动分析条件概率”到”主动设计生理结构先验”的转变。以下核心决策已确认并落地：

1. shared/private → source/observation ✅
2. 单一 shared quantizer → 双 source codebook + coupling 结构先验 ✅
3. 单 decoder → 双 decoder 加法架构 + 三模式显式训练 ✅
4. 自由参数化 coupling → lag focus + joint smoothness 结构先验 ✅
5. 旧 proxy target 不再代表当前 branch target 合同 ✅
6. Croce 2017 particle filter 生成显式 source/observation target 缓存 ✅
7. 局部 anchor-scoped 输入（EEG 6ch + fNIRS 1ch highWL）✅

**当前状态**：source/observation 架构稳定，两个数据集的 Croce local source/observation target 缓存已生成完毕。当前工作焦点是使用 highWL-only fNIRS 输入启动 tokenizer 训练，在 Croce cache target 上验证 Gate 1-4。Branch target 的物理定义已通过 Croce 2017 joint state-space model 确定：clean EEG / clean fNIRS source target 由共享生理状态联合约束，observation target 为逐模态线性残差。
