...
> Status: Source/observation architecture stabilized; branch-target physical model under revalidation. Legacy proxy target paths remain only as candidate baselines, not as the current branch-target contract.
...
当前项目的 tokenizer 主线已经完成从“被动分析条件概率”到“主动设计生理结构先验”的转变，但**branch target 的物理定义仍在重审**。当前已经确认的核心决策只有：
...
**当前状态**：source/observation 架构稳定，但 branch target 物理模型仍处于筛选与求解优化阶段。当前工作焦点不是继续把某个 proxy target 当默认答案，而是验证哪一类物理模型能够同时满足：

1. 从 EEG 与 fNIRS 联合约束出 clean source；
2. 对称地产生 clean EEG 与 clean fNIRS；
3. 保持 `observation_target = raw - source_target` 的线性可用性。
