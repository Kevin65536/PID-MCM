...
concentration 鼓励每行集中（确定性耦合），smoothness 鼓励邻近 EEG token 有相似的耦合分布。两者互补而非冗余：concentration 控制行内熵，smoothness 控制行间相似度。
...
2. Gate 1 (Health) 不退化
3. concentration ratio > 1.5
4. 邻居 token 的 coupling JS 散度显著低于随机 token 对
