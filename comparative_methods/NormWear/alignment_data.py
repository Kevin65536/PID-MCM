"""Public-only synchronized data boundary for NormWear adapter alignment v2."""
...
UNSUPPORTED_TASKS = ("refed_regression",)
...
def load_config(path: str | Path) -> tuple[dict[str, Any], Path]:
    ...
    if value.get("method_id") != METHOD_ID or value.get("mode") != "public_audit_only":
        raise PermissionError("NormWear alignment config must remain public-only")
    ...
    if float(data.get("eeg_sample_rate_hz", -1)) != 200.0:
        raise ValueError("NormWear canonical EEG rate must remain 200 Hz")
    if float(data.get("fnirs_sample_rate_hz", -1)) != 10.0:
        raise ValueError("NormWear canonical fNIRS rate must remain 10 Hz")
    if data.get("required_modalities") != ["eeg", "fnirs_hbo", "fnirs_hbr"]:
        raise ValueError("NormWear requires synchronized EEG, HbO, and HbR")
    if data.get("delivered_order") != "eeg_then_fnirs_hbo_then_fnirs_hbr":
        raise ValueError("NormWear delivered modality order drifted")

    observation = value.get("observation_budget", {})
    if observation.get("alignment_profile") != "support_matched_direct":
        raise ValueError("supported NormWear tasks must be support matched")
    if float(observation.get("extra_pre_anchor_context_s", -1)) != 0.0:
        raise ValueError("NormWear may not read pre-anchor context")
    if float(observation.get("extra_post_interval_context_s", -1)) != 0.0:
        raise ValueError("NormWear may not read post-interval context")

    adapter = value.get("adapter", {})
    if int(adapter.get("model_input_sample_rate_hz", -1)) != 65:
        raise ValueError("NormWear executable input rate must remain 65 Hz")
    resampling = adapter.get("resampling", {})
    if resampling.get("implementation") != "scipy.signal.resample_poly":
        raise ValueError("NormWear resampling implementation drifted")
    if resampling.get("eeg_ratio_up_down") != [13, 40]:
        raise ValueError("NormWear EEG resampling ratio drifted")
    if resampling.get("fnirs_ratio_up_down") != [13, 2]:
        raise ValueError("NormWear fNIRS resampling ratio drifted")
    ...
    for task in SUPPORTED_TASKS:
        ...
        if task_config.get("supported") is not True:
            raise ValueError(f"supported NormWear task is missing: {task}")
    ...
    if refed.get("supported") is not False or not refed.get("unsupported_reason_code"):
        raise ValueError("unsupported REFED cell lacks a frozen reason code")
...
class PublicInventory:
    ...
    @property
    def delivered_channel_names(self) -> tuple[str, ...]:
        return (
            *(f"eeg:{name}" for name in self.eeg_channels),
            *(f"fnirs_hbo:{name}" for name in self.fnirs_locations),
            *(f"fnirs_hbr:{name}" for name in self.fnirs_locations),
        )
...
def load_public_inventory(config: Mapping[str, Any], *, task: str) -> PublicInventory:
    if task not in SUPPORTED_TASKS:
        raise KeyError(f"task is not supported by the NormWear v2 adapter: {task}")
...
def data_branch_fingerprints(config: Mapping[str, Any]) -> dict[str, str]:
    ...
    if missing:
        raise FileNotFoundError(f"NormWear data-branch evidence is missing: {missing}")
    return {name: sha256_file(path) for name, path in paths.items()}


class NormWearPublicView(Dataset[dict[str, Any]]):
    """Deliver a frozen synchronized real-channel inventory without fabrication."""
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        hbo_indices = np.asarray(
            [by_role["HbO"][name] for name in self.inventory.fnirs_locations],
            dtype=np.int64,
        )
        hbr_indices = np.asarray(
            [by_role["HbR"][name] for name in self.inventory.fnirs_locations],
            dtype=np.int64,
        )
        ...
        eeg = np.asarray(source["eeg"], dtype=np.float32)[eeg_indices, : self.eeg_samples]
        ...
        if eeg.shape != expected[0] or hbo.shape != expected[1] or hbr.shape != expected[1]:
            raise ValueError(f"NormWear window is too short for {join_key}")
        if not bool(np.isfinite(eeg).all() and np.isfinite(hbo).all() and np.isfinite(hbr).all()):
            raise ValueError(f"NormWear input contains non-finite values: {join_key}")
        ...
        return {
            ...
            "delivered_channel_names": self.inventory.delivered_channel_names,
