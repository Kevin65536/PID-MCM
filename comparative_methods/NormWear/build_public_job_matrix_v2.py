#!/usr/bin/env python3
"""Build the reviewed, NormWear-only public-development v2 job matrix."""
...
MATRIX_SCHEMA = "normwear_public_job_matrix_candidate_v2"
...
def load_protocol(path: str | Path) -> tuple[dict[str, Any], Path]:
    ...
    if value.get("method_id") != METHOD_ID or value.get("mode") != "public_development_only":
        raise PermissionError("job matrix must remain NormWear public development only")
    ...
    if int(value["matrix"]["max_concurrent_jobs"]) != 1:
        raise ValueError("NormWear public jobs must remain serial")
    if int(value["failure_policy"]["automatic_retry_count"]) != 0:
        raise ValueError("candidate matrix must retain failures without automatic retry")
    runner_path = resolve_repo_path(value["runner"]["path"])
    if not runner_path.is_file():
        raise FileNotFoundError(f"missing NormWear runner: {runner_path}")
    if sha256_file(runner_path) != str(value["runner"]["sha256"]):
    ...
    if list(matrix["task_order"]) != list(config["job_matrix"]["tasks"]):
        raise ValueError("matrix task order differs from reviewed runner config")
    if list(matrix["outer_fold_order"]) != list(config["job_matrix"]["outer_folds"]):
        raise ValueError("matrix fold order differs from reviewed runner config")
    if list(matrix["seed_order"]) != list(config["job_matrix"]["seeds"]):
        raise ValueError("matrix seed order differs from reviewed runner config")
    expected = (
        len(matrix["task_order"])
        * len(matrix["outer_fold_order"])
        * len(matrix["seed_order"])
    )
    if expected != int(matrix["expected_job_count"]):
        raise ValueError("matrix expected job count is inconsistent")
...
def build_matrix(protocol_path: str | Path = DEFAULT_PROTOCOL) -> dict[str, Any]:
        ...
        for outer_fold in matrix["outer_fold_order"]:
            for seed in matrix["seed_order"]:
                ...
                if "protected" in " ".join(command).lower():
                    raise PermissionError("NormWear public job crossed protected boundary")
                jobs.append(
                    {
                        "order": order,
                        "job_id": f"normwear__{task}__outer{outer_fold}__seed{seed}",
                        "task": str(task),
                ...
                order += 1
    if len(jobs) != int(matrix["expected_job_count"]):
        raise RuntimeError("generated NormWear public matrix has wrong job count")
    ...
    return {
        "schema": "normwear_public_job_matrix_v2",
