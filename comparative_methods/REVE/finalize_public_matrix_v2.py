#!/usr/bin/env python3
"""Finalize retained REVE A8 evidence after the serial public matrix completes."""
...
from comparative_methods.REVE.alignment_data import SUPPORTED_TASKS, stable_hash
...
def validate_matrix_completion() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    require(status.get("status") == "completed", "REVE matrix controller is not complete")
    require(int(status.get("expected_job_count", -1)) == 90, "unexpected job count")
    require(int(status.get("completed_job_count", -1)) == 90, "REVE matrix is incomplete")
    require(int(status.get("failed_job_count", -1)) == 0, "REVE matrix retained failures")
    require(status.get("failures") == [], "REVE matrix failure list is non-empty")
    require(status.get("next_job_order") == 90, "REVE controller did not reach queue end")
    require(status.get("max_concurrent_jobs") == 1, "REVE jobs were not serial")
    require(status.get("automatic_retry_count") == 0, "REVE jobs admitted retries")
    require(
        completed.get("schema") == "reve_public_matrix_completed_audit_v2",
        "bad completed-audit schema",
    )
    require(completed.get("status") == "pass", "REVE completed audit did not pass")
...
def completion_summary(
    ...
    for task in SUPPORTED_TASKS:
        ...
        tracks = {str(row["track"]) for row in reports}
    ...
    return {
        "schema": "reve_public_matrix_completion_summary_v2",
        "status": "pass",
        "method_id": "reve",
...
def update_alignment_summary(
    ...
    require(not missing_peers, f"frozen alignment peers are missing: {missing_peers}")
