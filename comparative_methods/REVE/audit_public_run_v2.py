#!/usr/bin/env python3
"""Audit completed REVE public-development v2 runs and retained artifacts."""
...
AUDIT_SCHEMA = "reve_public_development_audit_v2"
...
def expected_membership(
    *, mode: str, fold: Any, config: Mapping[str, Any]
    ...
    if mode != "smoke_only":
        raise RuntimeError(f"unknown REVE public run mode: {mode}")
...
def audit_run(
    ...
    *,
    ...
    try:
        ...
        raise PermissionError(f"run is outside the REVE public root: {resolved_run}") from exc
    ...
    require(manifest.get("schema") == RUN_SCHEMA, "unexpected REVE run schema")
    require(manifest.get("status") == "completed", "REVE public run is not complete")
    require(manifest.get("method_id") == "reve", "run method identity is not REVE")
    ...
    require(
        manifest["track"] == alignment["tasks"][task]["track"],
        "run pretraining-exposure track drifted",
    )
    ...
    require(
        tuple(predictions["dataset_index"].astype(int).tolist()) == tuple(validation_indices),
    ...
    rows = [fold.inventory.dataset.lightweight_metadata(int(index)) for index in validation_indices]
    ...
    require(checkpoint.get("method_id") == "reve", "checkpoint method is not REVE")
    require(
