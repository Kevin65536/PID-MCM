#!/usr/bin/env python3
"""Run one REVE public selection/full-public-refit job under protocol v2."""
...
from comparative_methods.REVE.audit_alignment_v2 import exclusive_gpu_lock
...
CONFIG_SCHEMA = "reve_public_development_v2"
RUN_SCHEMA = "reve_public_development_run_v2"
FEATURE_SCHEMA = "reve_full_public_feature_cache_v2"
CHECKPOINT_SCHEMA = "reve_public_refit_checkpoint_v2"
...
def load_runner_config(path: str | Path) -> tuple[dict[str, Any], Path, dict[str, Any], Path]:
    ...
    if config.get("method_id") != "reve" or config.get("mode") != "public_development_only":
        raise PermissionError("runner config must remain REVE public development only")
    ...
    if sha256_file(alignment_path) != str(config["alignment"]["config_sha256"]):
        raise RuntimeError("REVE alignment config fingerprint drifted")
    ...
    if sha256_file(evidence_path) != str(config["alignment"]["evidence_summary_sha256"]):
        raise RuntimeError("REVE alignment evidence fingerprint drifted")
    evidence = json.loads(evidence_path.read_text(encoding="utf-8"))
    if evidence.get("status") != config["alignment"]["required_status"]:
        raise RuntimeError("REVE alignment evidence lacks the reviewed A0-A7 status")
    if evidence.get("protected_test_opened", False):
        raise PermissionError("REVE alignment evidence reports protected access")
    ...
    if tasks != SUPPORTED_TASKS or folds != tuple(range(5)) or len(set(seeds)) != len(seeds):
        raise ValueError("REVE public job matrix differs from the reviewed task/fold contract")
    if int(matrix["expected_public_jobs"]) != len(tasks) * len(folds) * len(seeds):
        raise ValueError("REVE expected public job count is inconsistent")
...
def adapter_identity(metadata: Any, alignment_path: Path) -> dict[str, Any]:
    ...
    identity["position_path"] = portable_path(Path(metadata.position_path))
    identity["source_file_sha256"] = {
        "adapter": sha256_file(METHOD_ROOT / "adapters/reve.py"),
        "alignment_data": sha256_file(METHOD_ROOT / "alignment_data.py"),
        "checkpoint_model_code": sha256_file(
            METHOD_ROOT / "checkpoints/reve-base/modeling_reve.py"
        ),
        "checkpoint_position_code": sha256_file(
            METHOD_ROOT / "checkpoints/reve-positions/position_bank.py"
        ),
        "identity_and_representation_audit": sha256_file(
            METHOD_ROOT / "IDENTITY_AND_REPRESENTATION_AUDIT.md"
        ),
        "alignment_config": sha256_file(alignment_path),
    }
    identity.update(
        {
            "output_layer": "final_transformer_latent_tokens_after_identity_final_layer",
            "embedding_dim": 512,
            ...
            "trainable_parameter_boundary": "frozen_encoder_and_query_linear_probe_only",
        }
    )
    return identity
    ...
    *,
    ...
    value = {
        ...
        "feature_extraction": {
            ...
            "geometry_policy": str(alignment["data"]["geometry_policy"]),
            "embedding_dim": 512,
        },
...
def validate_feature_arrays(arrays: Mapping[str, np.ndarray], fold: PublicFold) -> None:
    ...
    if set(arrays) != required:
        raise RuntimeError(f"REVE feature cache arrays differ: {sorted(arrays)}")
    count = len(fold.public_indices)
    if arrays["features"].shape != (count, 512):
        raise RuntimeError("REVE feature cache has an invalid feature shape")
    if any(len(arrays[name]) != count for name in required - {"features"}):
        raise RuntimeError("REVE feature cache arrays have inconsistent row counts")
    if tuple(arrays["dataset_indices"].astype(int).tolist()) != fold.public_indices:
        raise RuntimeError("REVE feature cache does not cover the exact public inventory")
    if tuple(arrays["sample_ids"].astype(str).tolist()) != fold.public_sample_ids:
        raise RuntimeError("REVE feature cache sample identity order drifted")
    if not bool(np.isfinite(arrays["features"]).all()):
        raise RuntimeError("REVE feature cache contains non-finite values")
    if not bool((arrays["features"].std(axis=0, dtype=np.float64) > 1e-8).any()):
        raise RuntimeError("REVE feature cache is globally constant")
    ...
    *,
    ...
    if cache_path.is_file() and manifest_path.is_file():
        ...
        if manifest != identity:
            raise RuntimeError("REVE feature cache manifest differs from its identity")
    ...
    with torch.inference_mode():
        for batch_number, batch in enumerate(loader, start=1):
            ...
            if embedding.shape != (batch_size, 512) or not bool(
                ...
                raise RuntimeError("REVE feature extraction returned invalid output")
    ...
    rows = [fold.inventory.dataset.lightweight_metadata(int(index)) for index in dataset_indices]
    arrays = {
        "features": features,
        "targets": np.asarray(
...
def select_and_refit(
    *,
    ...
    for candidate_index, (learning_rate, weight_decay) in enumerate(
        ...
        if best is None:
            raise RuntimeError("REVE public selection produced no candidate checkpoint")
        candidate = {
        ...
        if selected is None or float(candidate["best_macro_f1"]) > float(
            ...
            selected = candidate
    if selected is None:
        raise RuntimeError("REVE public selection grid is empty")
    ...
    if set(refit_indices) != set(train_indices).union(validation_indices):
        raise RuntimeError("REVE refit membership differs from public train+validation")
    ...
    reload_head = nn.Linear(512, class_count).to(device)
    ...
    if not np.array_equal(refit_mean, reloaded["feature_mean"].numpy()):
        raise RuntimeError("weights-only reload changed the REVE refit feature mean")
    if not np.array_equal(refit_scale, reloaded["feature_scale"].numpy()):
        raise RuntimeError("weights-only reload changed the REVE refit feature scale")
    if not np.allclose(reloaded_logits, expected_refit_logits, rtol=1e-5, atol=1e-6):
        raise RuntimeError("weights-only reload changed REVE refit logits")
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.task not in config["job_matrix"]["tasks"]:
        raise ValueError(f"task is outside the frozen REVE matrix: {args.task}")
    if int(args.outer_fold) not in config["job_matrix"]["outer_folds"]:
        raise ValueError("outer fold is outside the frozen REVE matrix")
    if int(args.seed) not in config["job_matrix"]["seeds"]:
        raise ValueError("seed is outside the frozen REVE matrix")
    output_dir = resolve_repo_path(args.output_dir)
    run_root = resolve_repo_path(config["resources"]["run_root"])
    try:
        output_dir.relative_to(run_root)
    except ValueError as exc:
        raise PermissionError(f"REVE public output must remain under {run_root}") from exc
    if output_dir == run_root:
        raise ValueError("REVE output directory must identify one job below the run root")
    if output_dir.exists() and any(output_dir.iterdir()):
        raise FileExistsError(f"REVE public run output already exists: {output_dir}")
    ...
    if not torch.cuda.is_available() or not device_name.startswith("cuda:"):
        raise RuntimeError("REVE public development requires the frozen CUDA lane")
    ...
    with exclusive_gpu_lock(lock_path):
        ...
        if any(parameter.requires_grad for parameter in model.parameters()):
            raise RuntimeError("REVE public runner found trainable encoder parameters")
        ...
        del model, encoder, position_bank
    ...
    manifest = {
        ...
        "seed": int(args.seed),
        "track": str(alignment["tasks"][args.task]["track"]),
        "device": device_name,
