#!/usr/bin/env python3
"""Audit completed CBraMod public-development v2 runs and retained artifacts."""
...
AUDIT_SCHEMA = "cbramod_public_development_audit_v2"
...
def expected_membership(
    *,
    ...
    if mode != "smoke_only":
        raise RuntimeError(f"unknown CBraMod public run mode: {mode}")
...
def audit_run(
    ...
    *,
    ...
    try:
        ...
        raise PermissionError(f"run is outside the CBraMod public root: {resolved_run}") from exc
    ...
    require(manifest.get("schema") == RUN_SCHEMA, "unexpected CBraMod run schema")
    require(manifest.get("status") == "completed", "CBraMod public run is not complete")
    require(manifest.get("method_id") == "cbramod", "run method identity is not CBraMod")
    ...
    require(checkpoint.get("method_id") == "cbramod", "checkpoint method is not CBraMod")
    require(
