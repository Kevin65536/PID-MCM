#!/usr/bin/env python3
"""Execute an explicitly authorized CBraMod public matrix one job at a time."""
...
from comparative_methods.CBraMod.audit_public_run_v2 import audit_run, utc_now
...
LAUNCH_SCHEMA = "cbramod_public_matrix_launch_v2"
MATRIX_SCHEMA = "cbramod_public_job_matrix_v2"
...
def load_launch(path: str | Path) -> tuple[dict[str, Any], Path, dict[str, Any], Path]:
    ...
    if launch.get("method_id") != "cbramod" or launch.get("mode") != "public_development_only":
        raise PermissionError("launch must remain CBraMod public development only")
    authorization = launch.get("authorization", {})
    if authorization.get("public_matrix_launch_authorized") is not True:
        raise PermissionError("CBraMod public matrix launch is not explicitly authorized")
    if authorization.get("protected_evaluation_authorized") is not False:
        raise PermissionError("CBraMod public controller cannot authorize protected evaluation")
    if authorization.get("reve_work_authorized") is not False:
        raise PermissionError("CBraMod public launch cannot authorize concurrent REVE work")
    ...
    if int(launch["execution"]["max_concurrent_jobs"]) != 1:
        raise ValueError("CBraMod matrix controller requires max_concurrent_jobs=1")
    if int(launch["execution"]["automatic_retry_count"]) != 0:
        raise ValueError("CBraMod matrix controller does not admit automatic retries")
    pilot_path = resolve_repo_path(launch["pilot_evidence"]["path"])
    if sha256_file(pilot_path) != str(launch["pilot_evidence"]["sha256"]):
        raise RuntimeError("CBraMod public pilot evidence fingerprint drifted")
    pilot = load_mapping(pilot_path)
    if pilot.get("status") != launch["pilot_evidence"]["required_status"]:
        raise RuntimeError("CBraMod public pilot does not have the required reviewed status")
    if pilot.get("protected_test_opened") is not False:
        raise PermissionError("CBraMod public pilot reports protected access")
    worker_path = resolve_repo_path(launch["controller"]["path"])
    if worker_path != Path(__file__).resolve():
        raise RuntimeError("launch manifest names a different CBraMod controller")
    if sha256_file(worker_path) != str(launch["controller"]["sha256"]):
        raise RuntimeError("CBraMod controller source fingerprint drifted")
    matrix_path = resolve_repo_path(launch["matrix"]["path"])
    if sha256_file(matrix_path) != str(launch["matrix"]["sha256"]):
        raise RuntimeError("CBraMod candidate matrix file fingerprint drifted")
    matrix = load_mapping(matrix_path)
    if matrix.get("schema") != MATRIX_SCHEMA or matrix.get("method_id") != "cbramod":
    ...
    if int(matrix.get("job_count", -1)) != 90:
        raise ValueError("CBraMod launch requires exactly 90 public jobs")
    if int(matrix.get("max_concurrent_jobs", -1)) != 1:
        raise ValueError("retained CBraMod matrix is not serial")
    if int(matrix.get("automatic_retry_count", -1)) != 0:
        raise ValueError("retained CBraMod matrix admits retries")
    ...
    if sha256_file(runner_path) != str(launch["runner"]["sha256"]):
        raise RuntimeError("CBraMod runner source fingerprint drifted")
    if sha256_file(config_path) != str(launch["runner"]["config_sha256"]):
        raise RuntimeError("CBraMod runner config fingerprint drifted")
...
def validate_jobs(
    ...
    if len(jobs) != int(matrix["job_count"]):
        raise ValueError("CBraMod matrix job list length drifted")
    if [int(job["order"]) for job in jobs] != list(range(len(jobs))):
        raise ValueError("CBraMod matrix job order is not contiguous")
    job_ids = [str(job["job_id"]) for job in jobs]
    if len(set(job_ids)) != len(job_ids):
        raise ValueError("CBraMod matrix contains duplicate job identities")
    ...
    if len(set(output_dirs)) != len(output_dirs):
        raise ValueError("CBraMod matrix contains duplicate output directories")
    return jobs
...
def controller_status(
    *,
    ...
    return {
        "schema": "cbramod_public_matrix_controller_status_v2",
        "status": state,
...
def execute(launch_path: str | Path, *, dry_run: bool) -> dict[str, Any]:
    ...
    if dry_run:
        return {
            "schema": "cbramod_public_matrix_controller_dry_run_v2",
            "status": "pass",
            "job_count": len(jobs),
            "max_concurrent_jobs": 1,
            "automatic_retry_count": 0,
    ...
    write_json(
        audit_path,
        {
            "schema": "cbramod_public_matrix_completed_audit_v2",
            "status": "pass",
            "matrix_identity_sha256": matrix["matrix_identity_sha256"],
            "job_count": len(jobs),
