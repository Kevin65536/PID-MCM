#!/usr/bin/env python3
"""Run one CBraMod public selection/full-public-refit job under protocol v2."""
...
CONFIG_SCHEMA = "cbramod_public_development_v2"
RUN_SCHEMA = "cbramod_public_development_run_v2"
FEATURE_SCHEMA = "cbramod_full_public_feature_cache_v2"
CHECKPOINT_SCHEMA = "cbramod_public_refit_checkpoint_v2"
...
def exclusive_gpu_lock(path: Path) -> Iterator[None]:
    ...
    with path.open("a+", encoding="utf-8") as handle:
        try:
            ...
            raise RuntimeError(f"CBraMod public GPU lane is already locked: {path}") from exc
        handle.seek(0)
        handle.truncate()
...
def load_runner_config(path: str | Path) -> tuple[dict[str, Any], Path, dict[str, Any], Path]:
    ...
    if config.get("method_id") != "cbramod" or config.get("mode") != "public_development_only":
        raise PermissionError("runner config must remain CBraMod public development only")
    ...
    if sha256_file(alignment_path) != str(config["alignment"]["config_sha256"]):
        raise RuntimeError("CBraMod alignment config fingerprint drifted")
    ...
    if sha256_file(evidence_path) != str(config["alignment"]["evidence_summary_sha256"]):
        raise RuntimeError("CBraMod alignment evidence fingerprint drifted")
    evidence = json.loads(evidence_path.read_text(encoding="utf-8"))
    if evidence.get("status") != config["alignment"]["required_status"]:
        raise RuntimeError("CBraMod alignment evidence does not have the required reviewed status")
    if evidence.get("protected_test_opened", False):
        raise PermissionError("CBraMod alignment evidence reports protected access")
    ...
    if tasks != SUPPORTED_TASKS or folds != tuple(range(5)) or len(set(seeds)) != len(seeds):
        raise ValueError("CBraMod public job matrix differs from the reviewed task/fold contract")
    if int(matrix["expected_public_jobs"]) != len(tasks) * len(folds) * len(seeds):
        raise ValueError("CBraMod expected public job count is inconsistent")
...
def adapter_identity(metadata: Any, alignment_path: Path) -> dict[str, Any]:
    ...
    identity["source_file_sha256"] = {
        "adapter": sha256_file(METHOD_ROOT / "adapters/cbramod.py"),
        "alignment_data": sha256_file(METHOD_ROOT / "alignment_data.py"),
        "upstream_model": sha256_file(METHOD_ROOT / "upstream/models/cbramod.py"),
        "upstream_transformer": sha256_file(
            METHOD_ROOT / "upstream/models/criss_cross_transformer.py"
        ),
        "representation_layer_audit": sha256_file(
            METHOD_ROOT / "REPRESENTATION_LAYER_AUDIT.md"
        ),
        "alignment_config": sha256_file(alignment_path),
    }
    identity.update(
        {
            "output_layer": "encoder_latent_before_pretraining_proj_out",
            "embedding_dim": 200,
            "pooling": "official_avgpooling_patch_reps",
    ...
    *,
    ...
    if cache_path.is_file() and manifest_path.is_file():
        ...
        if manifest != identity:
            raise RuntimeError("CBraMod feature cache manifest differs from its identity")
    ...
    with torch.inference_mode():
        for batch_number, batch in enumerate(loader, start=1):
            ...
            if embedding.shape != (batch_size, 200) or not bool(torch.isfinite(embedding).all()):
                raise RuntimeError("CBraMod feature extraction returned invalid output")
...
def validate_feature_arrays(arrays: Mapping[str, np.ndarray], fold: PublicFold) -> None:
    ...
    if set(arrays) != required:
        raise RuntimeError(f"CBraMod feature cache arrays differ: {sorted(arrays)}")
    count = len(fold.public_indices)
    if arrays["features"].shape != (count, 200):
        raise RuntimeError("CBraMod feature cache has an invalid feature shape")
    if any(len(arrays[name]) != count for name in required - {"features"}):
        raise RuntimeError("CBraMod feature cache arrays have inconsistent row counts")
    if tuple(arrays["dataset_indices"].astype(int).tolist()) != fold.public_indices:
        raise RuntimeError("CBraMod feature cache does not cover the exact public inventory")
    if tuple(arrays["sample_ids"].astype(str).tolist()) != fold.public_sample_ids:
        raise RuntimeError("CBraMod feature cache sample identity order drifted")
    if not bool(np.isfinite(arrays["features"]).all()):
        raise RuntimeError("CBraMod feature cache contains non-finite values")
    if not bool((arrays["features"].std(axis=0, dtype=np.float64) > 1e-8).any()):
        raise RuntimeError("CBraMod feature cache is globally constant")
    ...
    for epoch in range(1, int(epochs) + 1):
        ...
        for start in range(0, len(permutation), int(batch_size)):
            ...
            if not bool(torch.isfinite(loss)):
                raise FloatingPointError("CBraMod linear probe produced non-finite loss")
            loss.backward()
            optimizer.step()
...
def select_and_refit(
    *,
    ...
    for candidate_index, (learning_rate, weight_decay) in enumerate(
        ...
        if best is None:
            raise RuntimeError("CBraMod public selection produced no candidate checkpoint")
        candidate = {
        ...
        if selected is None or float(candidate["best_macro_f1"]) > float(
            ...
            selected = candidate
    if selected is None:
        raise RuntimeError("CBraMod public selection grid is empty")
    ...
    if set(refit_indices) != set(train_indices).union(validation_indices):
        raise RuntimeError("CBraMod refit membership differs from public train+validation")
    ...
    reload_head = nn.Linear(200, class_count).to(device)
    ...
    if not np.array_equal(refit_mean, reloaded["feature_mean"].numpy()):
        raise RuntimeError("weights-only reload changed the CBraMod refit feature mean")
    if not np.array_equal(refit_scale, reloaded["feature_scale"].numpy()):
        raise RuntimeError("weights-only reload changed the CBraMod refit feature scale")
    if not np.allclose(reloaded_logits, expected_refit_logits, rtol=1e-5, atol=1e-6):
        raise RuntimeError("weights-only reload changed CBraMod refit logits")
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.task not in config["job_matrix"]["tasks"]:
        raise ValueError(f"task is outside the frozen CBraMod matrix: {args.task}")
    if int(args.outer_fold) not in config["job_matrix"]["outer_folds"]:
        raise ValueError("outer fold is outside the frozen CBraMod matrix")
    if int(args.seed) not in config["job_matrix"]["seeds"]:
        raise ValueError("seed is outside the frozen CBraMod matrix")
    output_dir = resolve_repo_path(args.output_dir)
    run_root = resolve_repo_path(config["resources"]["run_root"])
    try:
        output_dir.relative_to(run_root)
    except ValueError as exc:
        raise PermissionError(f"CBraMod public output must remain under {run_root}") from exc
    if output_dir == run_root:
        raise ValueError("CBraMod output directory must identify one job below the run root")
    if output_dir.exists() and any(output_dir.iterdir()):
        raise FileExistsError(f"CBraMod public run output already exists: {output_dir}")
    ...
    if not torch.cuda.is_available() or not device_name.startswith("cuda:"):
        raise RuntimeError("CBraMod public development requires the frozen CUDA lane")
    ...
    with exclusive_gpu_lock(lock_path):
        ...
        if any(parameter.requires_grad for parameter in model.parameters()):
            raise RuntimeError("CBraMod public runner found trainable encoder parameters")
