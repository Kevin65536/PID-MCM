...
from comparative_methods.CBraMod.build_public_job_matrix_v2 import build_matrix
...
def test_public_runner_config_freezes_one_cbramod_matrix() -> None:
    ...
    assert alignment["method_id"] == "cbramod"
    assert config["protected_test_default"] == "locked"
...
def test_public_runner_refuses_output_outside_cbramod_run_root(tmp_path: Path) -> None:
...
def test_completed_public_matrix_remains_terminal_after_queue_completion() -> None:
    ...
    assert queue["cbramod"]["current_state"] == (
