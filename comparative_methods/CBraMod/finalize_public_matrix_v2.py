#!/usr/bin/env python3
"""Finalize retained CBraMod A8 evidence after the serial public matrix completes."""
...
from comparative_methods.CBraMod.alignment_data import SUPPORTED_TASKS, stable_hash
...
def validate_matrix_completion() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    require(status.get("status") == "completed", "CBraMod matrix controller is not complete")
    require(int(status.get("expected_job_count", -1)) == 90, "unexpected job count")
    require(int(status.get("completed_job_count", -1)) == 90, "CBraMod matrix is incomplete")
    require(int(status.get("failed_job_count", -1)) == 0, "CBraMod matrix retained failures")
    require(status.get("failures") == [], "CBraMod matrix failure list is non-empty")
    require(status.get("next_job_order") == 90, "CBraMod controller did not reach queue end")
    require(status.get("max_concurrent_jobs") == 1, "CBraMod jobs were not serial")
    require(status.get("automatic_retry_count") == 0, "CBraMod jobs admitted retries")
    require(completed.get("schema") == "cbramod_public_matrix_completed_audit_v2", "bad audit schema")
    require(completed.get("status") == "pass", "CBraMod completed audit did not pass")
...
def completion_summary(status: Mapping[str, Any], completed: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema": "cbramod_public_matrix_completion_summary_v2",
        "status": "pass",
        "method_id": "cbramod",
...
def update_alignment_summary(cell_paths: list[Path], completion: Mapping[str, Any]) -> None:
    ...
    peer_paths = [
        BIOT_PEER_ROOT / f"{task}.json"
        for task in (*SUPPORTED_TASKS, "refed_regression")
    ]
    missing_peers = [str(path) for path in peer_paths if not path.is_file()]
    require(not missing_peers, f"frozen BIOT alignment peers are missing: {missing_peers}")
    schema_report = audit_alignment(ALIGNMENT_CONTRACT, [*peer_paths, *cell_paths])
