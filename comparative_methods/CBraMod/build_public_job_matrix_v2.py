#!/usr/bin/env python3
"""Build the reviewed, CBraMod-only public-development v2 job matrix."""
...
MATRIX_SCHEMA = "cbramod_public_job_matrix_candidate_v2"
...
def load_protocol(path: str | Path) -> tuple[dict[str, Any], Path]:
    ...
    if value.get("method_id") != "cbramod" or value.get("mode") != "public_development_only":
        raise PermissionError("job matrix must remain CBraMod public development only")
    if value.get("protected_test_default") != "locked":
        raise PermissionError("protected test must remain locked")
    if value.get("authorization", {}).get("protected_evaluation_authorized") is not False:
        raise PermissionError("candidate matrix may not authorize protected evaluation")
    if value.get("authorization", {}).get("public_matrix_launch_authorized") is not False:
        raise PermissionError("candidate matrix may not launch itself")
    if int(value["matrix"]["max_concurrent_jobs"]) != 1:
        raise ValueError("CBraMod public jobs must remain serial")
    if int(value["failure_policy"]["automatic_retry_count"]) != 0:
        raise ValueError("CBraMod candidate matrix must retain failures without automatic retry")
    runner_path = resolve_repo_path(value["runner"]["path"])
    if not runner_path.is_file():
        raise FileNotFoundError(f"missing CBraMod runner: {runner_path}")
...
def build_matrix(protocol_path: str | Path = DEFAULT_PROTOCOL) -> dict[str, Any]:
    ...
    for task in matrix["task_order"]:
        for outer_fold in matrix["outer_fold_order"]:
            for seed in matrix["seed_order"]:
                ...
                require_text = " ".join(command).lower()
                if "protected" in require_text:
                    raise PermissionError("CBraMod public job command crossed protected boundary")
                jobs.append(
                    {
                        "order": order,
                        "job_id": f"cbramod__{task}__outer{outer_fold}__seed{seed}",
                        "task": str(task),
    ...
    if len(jobs) != int(matrix["expected_job_count"]):
        raise RuntimeError("generated CBraMod public matrix has the wrong job count")
    ...
    return {
        "schema": "cbramod_public_job_matrix_v2",
