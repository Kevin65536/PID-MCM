#!/usr/bin/env python3
"""Execute an explicitly authorized BIOT public matrix one job at a time."""
...
from comparative_methods.BIOT.audit_public_run_v2 import audit_run, utc_now
...
LAUNCH_SCHEMA = "biot_public_matrix_launch_v2"
MATRIX_SCHEMA = "biot_public_job_matrix_v2"
...
def load_launch(path: str | Path) -> tuple[dict[str, Any], Path, dict[str, Any], Path]:
    ...
    if launch.get("method_id") != "biot" or launch.get("mode") != "public_development_only":
        raise PermissionError("launch must remain BIOT public development only")
    authorization = launch.get("authorization", {})
    if authorization.get("public_matrix_launch_authorized") is not True:
        raise PermissionError("BIOT public matrix launch is not explicitly authorized")
    if authorization.get("protected_evaluation_authorized") is not False:
        raise PermissionError("BIOT public controller cannot authorize protected evaluation")
    ...
    if sha256_file(worker_path) != str(launch["controller"]["sha256"]):
        raise RuntimeError("BIOT controller source fingerprint drifted")
    matrix_path = resolve_repo_path(launch["matrix"]["path"])
    if sha256_file(matrix_path) != str(launch["matrix"]["sha256"]):
        raise RuntimeError("BIOT candidate matrix file fingerprint drifted")
    matrix = load_mapping(matrix_path)
    if matrix.get("schema") != MATRIX_SCHEMA or matrix.get("method_id") != "biot":
    ...
    if int(matrix.get("max_concurrent_jobs", -1)) != 1:
        raise ValueError("retained BIOT matrix is not serial")
    if int(matrix.get("automatic_retry_count", -1)) != 0:
        raise ValueError("retained BIOT matrix admits retries")
...
def validate_jobs(
    ...
    if len(jobs) != int(matrix["job_count"]):
        raise ValueError("BIOT matrix job list length drifted")
    if [int(job["order"]) for job in jobs] != list(range(len(jobs))):
        raise ValueError("BIOT matrix job order is not contiguous")
    job_ids = [str(job["job_id"]) for job in jobs]
    if len(set(job_ids)) != len(job_ids):
        raise ValueError("BIOT matrix contains duplicate job identities")
    ...
    if len(set(output_dirs)) != len(output_dirs):
        raise ValueError("BIOT matrix contains duplicate output directories")
    return jobs
...
def controller_status(
    *,
    ...
    return {
        "schema": "biot_public_matrix_controller_status_v2",
        "status": state,
...
def execute(launch_path: str | Path, *, dry_run: bool) -> dict[str, Any]:
    ...
    if dry_run:
        return {
            "schema": "biot_public_matrix_controller_dry_run_v2",
            "status": "pass",
            "job_count": len(jobs),
            "max_concurrent_jobs": 1,
            "automatic_retry_count": 0,
    ...
    write_json(
        audit_path,
        {
            "schema": "biot_public_matrix_completed_audit_v2",
            "status": "pass",
            "matrix_identity_sha256": matrix["matrix_identity_sha256"],
            "job_count": len(jobs),
