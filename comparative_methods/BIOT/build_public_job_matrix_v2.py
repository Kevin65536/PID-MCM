#!/usr/bin/env python3
"""Build the reviewed, BIOT-only public-development v2 job matrix."""
...
MATRIX_SCHEMA = "biot_public_job_matrix_candidate_v2"
...
def load_protocol(path: str | Path) -> tuple[dict[str, Any], Path]:
    ...
    if value.get("method_id") != "biot" or value.get("mode") != "public_development_only":
        raise PermissionError("job matrix must remain BIOT public development only")
    ...
    if int(value["matrix"]["max_concurrent_jobs"]) != 1:
        raise ValueError("BIOT public jobs must remain serial")
    if int(value["failure_policy"]["automatic_retry_count"]) != 0:
        raise ValueError("BIOT candidate matrix must retain failures without automatic retry")
    runner_path = resolve_repo_path(value["runner"]["path"])
    if not runner_path.is_file():
        raise FileNotFoundError(f"missing BIOT runner: {runner_path}")
...
def build_matrix(protocol_path: str | Path = DEFAULT_PROTOCOL) -> dict[str, Any]:
        ...
        for outer_fold in matrix["outer_fold_order"]:
            for seed in matrix["seed_order"]:
                ...
                if "protected" in require_text:
                    raise PermissionError("BIOT public job command crossed protected boundary")
                jobs.append(
                    {
                        "order": order,
                        "job_id": f"biot__{task}__outer{outer_fold}__seed{seed}",
                        "task": str(task),
    ...
    if len(jobs) != int(matrix["expected_job_count"]):
        raise RuntimeError("generated BIOT public matrix has the wrong job count")
    ...
    return {
        "schema": "biot_public_job_matrix_v2",
