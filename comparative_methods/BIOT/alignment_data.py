"""BIOT-owned public data boundary for adapter-alignment v2 auditing."""
...
CONFIG_SCHEMA = "biot_adapter_alignment_v2"
SUPPORTED_TASKS = (
    "motor_imagery",
    "mental_arithmetic",
    "wg",
...
def load_config(path: str | Path) -> tuple[dict[str, Any], Path]:
    ...
    if value.get("method_id") != "biot" or value.get("mode") != "public_audit_only":
        raise PermissionError("BIOT alignment config must remain a public-only BIOT audit")
    ...
    if int(value["data"]["panel_size"]) != 16:
        raise ValueError("BIOT PREST-16 audit requires a 16-channel panel")
    for task in SUPPORTED_TASKS:
        task_config = value.get("tasks", {}).get(task)
        if not isinstance(task_config, dict) or task_config.get("supported") is not True:
            raise ValueError(f"supported BIOT task is missing from config: {task}")
...
def load_public_inventory(
    ...
    if task not in SUPPORTED_TASKS:
        raise KeyError(f"task is not supported by the BIOT v2 audit: {task}")
...
class BIOTPublicView(Dataset[dict[str, Any]]):
    """Deliver only the declared real EEG panel and full recorded support."""
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        if missing:
            raise ValueError(f"BIOT panel absent for {join_key}: {missing}")
        ...
        if bool(bad.any()):
            ...
            raise ValueError(f"BIOT panel contains bad measured channels for {join_key}: {rejected}")
        ...
        if missing_geometry:
            raise ValueError(f"BIOT panel lacks registered geometry: {missing_geometry}")
...
def data_branch_fingerprints(config: Mapping[str, Any]) -> dict[str, str]:
    ...
    if missing:
        raise FileNotFoundError(f"BIOT data branch evidence is missing: {missing}")
