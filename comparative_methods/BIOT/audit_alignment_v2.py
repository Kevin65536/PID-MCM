#!/usr/bin/env python3
"""Run BIOT's full-public adapter-alignment v2 audit without protected reads."""
...
def exclusive_gpu_lock(path: Path) -> Iterator[None]:
    ...
    with path.open("a+", encoding="utf-8") as handle:
        try:
            ...
            raise RuntimeError(f"BIOT alignment GPU lane is already locked: {path}") from exc
        handle.seek(0)
        handle.truncate()
    ...
    identity.update(
        {
            ...
            "patch_and_token_grid": {
                "stft_n_fft": int(metadata.n_fft),
                "stft_hop_length": int(metadata.hop_length),
                "frames_per_channel": frames,
                "sequence_tokens": len(panel) * frames,
            },
            "geometry_encoding": "upstream_channel_index_token_for_declared_native_electrode_order",
            "pooling": "upstream_mean_over_channel_frequency_tokens",
            "deterministic_source_declared_sample_transform": "none_after_canonical_200hz_coordinate",
...
def run_task(
    *,
        ...
        for batch_number, batch in enumerate(loader, start=1):
            ...
            if batch_number == 1:
                ...
                if not deterministic_replay:
                    raise RuntimeError("BIOT deterministic replay differed for identical input")
            if embedding.shape != (batch_size, 256) or not bool(torch.isfinite(embedding).all()):
                raise RuntimeError(f"BIOT produced invalid public features: {tuple(embedding.shape)}")
    ...
    if len(all_sample_ids) != len(observed_ids) or observed_ids != expected_ids:
        raise RuntimeError(f"BIOT public coverage is incomplete or duplicated for {task}")
    if set(dataset_indices.tolist()) != set(inventory.indices):
        raise RuntimeError(f"BIOT public dataset-index coverage drifted for {task}")
    required_samples = int(round(inventory.duration_s * 200.0))
    if not bool((support_counts == required_samples).all()):
        raise RuntimeError(f"BIOT recorded-support counts drifted for {task}")
    ...
    if not bool(np.isfinite(features).all()) or not bool((coordinate_std > 1e-8).any()):
        raise RuntimeError(f"BIOT public features are non-finite or globally constant for {task}")
    ...
    evidence = {
        ...
        "cell_id": f"biot__{task}__support_matched_direct__v2",
...
def unsupported_refed_cell(
    *, config: Mapping[str, Any], alignment_contract: Mapping[str, Any]
    ...
    return {
        "schema": EVIDENCE_SCHEMA,
        "cell_id": "biot__refed_regression__support_matched_direct__v2",
...
def parse_tasks(values: Sequence[str]) -> tuple[str, ...]:
    ...
    if unknown:
        raise ValueError(f"unknown or unsupported BIOT audit tasks: {unknown}")
    if len(set(tasks)) != len(tasks):
        raise ValueError("BIOT audit tasks must be unique")
    return tasks
...
def main() -> int:
    ...
    if device.type == "cuda" and not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for the full BIOT alignment audit")
    ...
    with exclusive_gpu_lock(lock_path):
        ...
        if any(parameter.requires_grad for parameter in model.parameters()):
            raise RuntimeError("BIOT alignment audit found trainable encoder parameters")
        for task in tasks:
    ...
    if set(tasks) == set(SUPPORTED_TASKS):
    ...
    summary = {
        "schema": "biot_adapter_alignment_audit_report_v2",
