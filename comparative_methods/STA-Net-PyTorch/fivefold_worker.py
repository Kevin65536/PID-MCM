#!/usr/bin/env python3
"""Train and evaluate one queue from the frozen STA-Net five-fold benchmark."""
...
def freeze_fold(job: Mapping[str, Any], training_dir: Path, output: Path) -> None:
    public_path = Path(str(job["public_manifest"])).resolve()
    protected_path = Path(str(job["protected_manifest"])).resolve()
    config_path = Path(str(job["config"])).resolve()
    checkpoint_path = training_dir / "checkpoint_best.pt"
    ...
    if training.get("split_sha256") != public.get("split_sha256"):
        raise RuntimeError("training run does not match its public split")
    if sha256(config_path) != job["config_sha256"]:
        raise RuntimeError("selected config hash drifted")
    if sha256(public_path) != job["public_manifest_sha256"]:
        raise RuntimeError("public manifest hash drifted")
    if sha256(protected_path) != job["protected_manifest_sha256"]:
        raise RuntimeError("protected manifest hash drifted")
    payload = {
        "schema": "sta_net_frozen_tuning_winner_v1",
        "study_id": "sta_net_strict_vs_sample_random_5fold_v1",
        "study_name": (
            "frozen_cross_subject_selected_hyperparameters__"
            + str(job["protocol"])
        ),
        "task": str(job["task"]),
        "protocol": str(job["protocol"]),
        "fold_id": protected.get("fold_id"),
        "outer_fold": int(job["outer_fold"]),
        "trial_number": int(job["source_trial"]),
        ...
        "split_manifest": str(public_path),
        "split_manifest_sha256": sha256(public_path),
...
def main() -> None:
    ...
    if not args.unlock_protected_test:
        raise RuntimeError("five-fold protected evaluation requires explicit unlock")
    ...
    write_json(status_path, {
        "schema": "sta_net_fivefold_lane_v1",
        "status": "running",
    ...
    try:
        for position, job in enumerate(jobs):
            key = f"{job['protocol_key']}/{job['task']}/{job['fold_id']}"
            fold_dir = (
                root / "folds" / str(job["protocol_key"]) / str(job["task"]) / str(job["fold_id"])
            )
